...
var _ = Describe("MaintenanceWindow Controller", func() {
	Context("When reconciling a resource", func() {
		...
		BeforeEach(func() {
			...
			err := k8sClient.Get(ctx, namespacedName, mw)
			if err != nil && errors.IsNotFound(err) {
				resource := &uptimerobotv1.MaintenanceWindow{
					ObjectMeta: metav1.ObjectMeta{
						Name:      resourceName,
						Namespace: "default",
					},
					Spec: uptimerobotv1.MaintenanceWindowSpec{
						...
						StartTime:       "02:00:00",
						Duration:        metav1.Duration{Duration: 60 * 60 * 1000000000}, // 1 hour in nanoseconds

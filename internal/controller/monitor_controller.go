...
func (r *MonitorReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	for _, ref := range monitor.Spec.Contacts {
		...
		if err := GetContact(ctx, r.Client, contact, ref.Name); err != nil {
			recordMonitorError("contact_not_found")
			return ctrl.Result{}, err
		}

		if contact.Status.ID == "" {
			// Contact hasn't been reconciled yet - requeue without error. Only the
	...
	if auth := monitor.Spec.Monitor.Auth; auth != nil && auth.SecretName != "" {
		...
		if err := r.Get(ctx, client.ObjectKey{
			...
			recordMonitorError("secret_not_found")
			return ctrl.Result{}, err
		}

		val, ok := secret.Data[auth.UsernameKey]
		if !ok {
			recordMonitorError("secret_missing_key")
	...
	if monitor.Status.ID == "" {
		...
		if adoptID, hasAdoptID := monitor.Annotations[AdoptIDAnnotation]; hasAdoptID && adoptID != "" {
			...
			if err != nil {
				...
				if errors.Is(err, uptimerobot.ErrMonitorNotFound) {
					recordMonitorError("adoption_not_found")
			...
			if existingType != monitor.Spec.Monitor.Type {
				recordMonitorError("adoption_type_mismatch")
				monitor.Status.Ready = false
		...
		if currentMonitor != nil {
			...
			// - "UP": Monitor is running and responding successfully
			// - "DOWN", "SEEMS DOWN": Monitor is running but experiencing issues
			// Note: "STARTED" may be returned by the API in some cases and is treated as running
